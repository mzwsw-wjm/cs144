...
struct ReceiveFrame : public Action<NetworkInterface>
{
    ...
    void execute(NetworkInterface &interface) const override
    {
        ...
        if (result.has_value() and not expected.has_value()) {
            throw ExpectationViolation("an arriving Ethernet frame was passed up the stack as an Internet "
        ...
        if (expected.has_value() and not result.has_value()) {
            throw ExpectationViolation("an arriving Ethernet frame was expected to be passed up the stack as an "
                                       "Internet datagram, but wasn't");
        }

        if (not equal(result.value(), expected.value())) {
            throw ExpectationViolation(
                std::string(
                    "NetworkInterface::recv_frame() produced a different Internet datagram than was expected: ")
                + "actual={" + result.value().header.to_string() + "}");
        }
    }

    ReceiveFrame(EthernetFrame f, std::optional<InternetDatagram> e) : frame(std::move(f)), expected(std::move(e))
    {}
};
...
inline std::string concat(std::vector<Buffer> &buffers)
{
    return std::accumulate(buffers.begin(), buffers.end(), std::string {},
                           [](const std::string &x, const Buffer &y) { return x + static_cast<std::string>(y); });
}
