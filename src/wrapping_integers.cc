...
uint64_t Wrap32::unwrap(Wrap32 zero_point, uint64_t checkpoint) const
{
    ...
    if (checkpoint > seqno_offset) {
        ...
        const uint64_t abs_seqno_extra_part_offset = checkpoint - seqno_offset + (UINT32_SIZE >> 1);
        const uint64_t UINT32_SIZE_num = abs_seqno_extra_part_offset / UINT32_SIZE;
