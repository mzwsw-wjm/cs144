...
{
    if (segments_out_.size() > 0 && set_syn_) {
...
{
    uint64_t curr_window_size = window_size_ ? window_size_ : 1;
    while (curr_window_size > outstanding_seqno_) {
        ...
        if (!set_fin_ && outbound_stream.is_finished() && payload.size() + outstanding_seqno_ + msg.SYN < curr_window_size) {
            msg.FIN = true;
            set_fin_ = true;
        }
