...
template <typename AdaptT> void TCPMinnowSocket<AdaptT>::_tcp_loop(const function<bool()> &condition)
{
...
template <typename AdaptT> void TCPMinnowSocket<AdaptT>::_initialize_TCP(const TCPConfig &config)
{
    _tcp.emplace(config);
...
template <typename AdaptT> TCPMinnowSocket<AdaptT>::~TCPMinnowSocket()
{
    try {
...
template <typename AdaptT> void TCPMinnowSocket<AdaptT>::wait_until_closed()
{
    shutdown(SHUT_RDWR);
    if (_tcp_thread.joinable()) {
...
template <typename AdaptT> void TCPMinnowSocket<AdaptT>::_tcp_main()
{
    try {
        if (not _tcp.has_value()) {
...
template <typename AdaptT> void TCPMinnowSocket<AdaptT>::collect_segments()
{
    if (not _tcp.has_value()) {
        return;
    }
...
FullStackSocket::FullStackSocket()
    : TCPOverIPv4OverEthernetMinnowSocket(TCPOverIPv4OverEthernetAdapter(
        TapFD("tap10"), random_private_ethernet_address(), Address(LOCAL_TAP_IP_ADDRESS, "0"),
