...
EventLoop::FDRule::FDRule(BasicRule &&base, FileDescriptor &&s_fd, Direction s_direction, CallbackT s_cancel,
                          InterestT s_recover)
    : BasicRule(base), fd(move(s_fd)), direction(s_direction), cancel(move(s_cancel)), recover(move(s_recover))
{}
                                          ...
                                          const CallbackT &cancel, const InterestT &recover)
{
    ...
    _fd_rules.emplace_back(make_shared<FDRule>(BasicRule {category_id, interest, callback}, fd.duplicate(),
                                               direction, cancel, recover));
